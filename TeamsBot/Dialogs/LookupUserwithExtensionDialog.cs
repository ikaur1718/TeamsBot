...
{
    public class LookupUserwithExtensionDialog : LogoutDialog
    {
        protected readonly ILogger _logger;
        private readonly IGraphClientService _graphClientService;

        public LookupUserwithExtensionDialog(
        ...
        private async Task<DialogTurnResult> DisplayUserwithExtension(
        WaterfallStepContext stepContext,
        CancellationToken cancellationToken)
        {
            await stepContext.BeginDialogAsync(nameof(OAuthPrompt), null, cancellationToken);

            return await stepContext.PromptAsync("extension",
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("What's the phone's extension for look up?")
                },
                cancellationToken);
        }
        ...
        CancellationToken cancellationToken)
        {
            ...
            {
                stepContext.Values["extension"] = (string)stepContext.Result;
        ...
        CancellationToken cancellationToken)
        {
            if (stepContext.Result != null)
            {
                ...
                if (tokenResponse?.Token != null)
                {
                    ...
                    var user = phones.Where(t => t.BusinessPhones.Contains(stepContext.Values["extension"])).Select(u => u.DisplayName);
